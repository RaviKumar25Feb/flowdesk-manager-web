...
} from "../../services/projectComment.service";
...
const ProjectComments = ({ projectId }) => {
  ...
  const fetchComments = async () => {
    try {
      ...
      const response = await getProjectDiscussions(projectId);
  ...
  const handleCreateComment = async (event) => {
    ...
    try {
      ...
      const response = await createProjectDiscussion(projectId, {
        message,
      });
      ...
      toast.error(
        error.response?.data?.message || "Failed to add project comment.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleUpdateComment = async (commentId) => {
    ...
    try {
      ...
      const response = await updateProjectDiscussion(commentId, {
        message,
      });
      ...
      toast.error(
        error.response?.data?.message || "Failed to update project comment.",
      );
    } finally {
      setUpdating(false);
    }
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      ...
      await deleteProjectDiscussion(commentId);
      ...
      toast.error(
        error.response?.data?.message || "Failed to delete project comment.",
      );
    } finally {
      setDeletingId(null);
    }
    ...
    <section className="rounded-md border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-gray-900">Project Discussion</h2>

            <p className="text-xs text-gray-500">
              Manager & Client Discussions
            </p>
          </div>
        </div>
  ...
  return (
    <div className="rounded-xl border border-dashed border-gray-300 py-10 text-center">
      ...
      <p className="mt-1 text-xs text-gray-500">
        Add the first comment for this project.
      </p>
    </div>
  );
};
